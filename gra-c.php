...
if ( $m2 != 0)
{
	...
	mysql_query("update rev set name = 'Na danej pozycji już jest pionek. Podaj inną pozycję' ");
		...
		mysql_query("update reversi	set V=2 where X=$xx and Y=$yy");
		mysql_query("update move set move='white' ");
		mysql_query("update rev set name = 'Twoj Ruch' ");
		$errorb = "ruch wykonany";
		
}
echo $errorb;
//<meta http-equiv="refresh" content="0;url=index-black.php" />
// funkcja wygranej
		...
		for ($yy2; $yy2>=0; $yy2--){
			for ($xx2; $xx2<8; $xx2++){
					...
					if ($gra2 ==2){
						...
						if ($gra2 ==1){
							mysql_query("update reversi set V=2 where X=$xx2 and Y=$yy2");
												}
							}
															};
														};
...
echo "bład";
	...
	//	for ($yy2; $yy2>=0; $yy2--){
			...
			if ($gra22 ==2){
				...
				if ($gra22 ==1){
					mysql_query("update reversi set V=2 where X=$xx3 and Y=$yy3");
										}
			}
			};
			//};
			...
			for ($xx4; $xx4<8; $xx4++){
				$gra111 =mysql_query("select V from reversi where X=$xx4 and Y=$yy4 "); $gra222= mysql_result($gra111,0,0);
					...
					echo "błąd";
						...
						if ($gra22 ==1){
					mysql_query("update reversi set V=2 where X=$xx4 and Y=$yy4");
										}
			}
			};
			};
...
echo "bład";
	...
	for ($yy5; $yy5<8; $yy5++){
			...
			if ($gra2222 ==2){
				...
				if ($gra2222 ==1){
					mysql_query("update reversi set V=2 where X=$xx5 and Y=$yy5");
										}
			}
			};
			//};
...
echo "bład";
	...
	for ($yy6; $yy6>=0; $yy6--){
			...
			if ($gra22222 ==2){
				...
				if ($gra22222 ==1){
					mysql_query("update reversi set V=2 where X=$xx6 and Y=$yy6");
										}
			}
			};
			};
...
echo "bład";
	...
	for ($yy8; $yy8>=0; $yy8--){
			...
			if ($gra22222 ==2){
				...
				if ($gra22222 ==1){
					mysql_query("update reversi set V=2 where X=$xx8 and Y=$yy8");
										}
			}
			};
	};
